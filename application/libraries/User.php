...
class User
{
	...
	public function update()
	{
		...
		{
			$this->CI->session->set_flashdata('message', alert_danger('You don\'t have permission to update the user', 'text-center'));
			redirect('dashboard/user');
		}
		else
		{
	...
	private function _update_action($user)
	{
		if(input_post('action') == 'update')
		{
			...
			// $this->CI->form_validation->set_rules('level', 'Level', array(
			// 	'trim', 'required'
			// ));
			...
			if($this->CI->form_validation->run())
			{
				...
				if(input_post('password', '') != '')
					$data_user['password'] = sha(input_post('password'));
