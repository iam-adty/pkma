...
class Blog
{
			...
			if($this->CI->form_validation->run())
			{
				...
				else
				{
					$this->CI->session->set_flashdata('message', alert_success('New post created!', 'text-center'));
						redirect('dashboard/custom/blog');
				}
			}
			else
	...
	public function update()
	{
		...
		$data = array(
			...
			'author_name' => set_value('author_name', $page['post_author_name']),
			'author_id' => set_value('author_id', $page['post_author_id']),
			'content_message' => text_danger(form_error('content')),
			'title_message' => text_danger(form_error('title')),
	...
	private function _update_action()
	{
		if(input_post('action') == 'update')
		{
			...
			if($this->CI->form_validation->run())
			{
				...
				if(in_array('dashboard_custom_blog_update_other', $this->CI->session->userdata('blog_user')['access']))
