...
class Model_page extends MY_Model
{
	...
	private function _join()
	{
		...
		if(in_array('dashboard_page_list', $this->session->userdata('blog_user')['access']))
		{
			if(!in_array('revoke_dashboard_page_list', $this->session->userdata('blog_user')['access']))
			{
				$own = FALSE;
				if(in_array('dashboard_page_list_own', $this->session->userdata('blog_user')['access']))
				{
					if(!in_array('revoke_dashboard_page_list_own', $this->session->userdata('blog_user')['access']))
					{
						$own = TRUE;
				...
				if(in_array('dashboard_page_list_other', $this->session->userdata('blog_user')['access']))
				{
					if(!in_array('revoke_dashboard_page_list_other', $this->session->userdata('blog_user')['access']))
					{
						if($own)
						{
